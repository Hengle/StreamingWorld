namespace MightyTerrainMesh
{
    ...
    public static class MTFileUtils
    {
        ...
        {
            string path = string.Format("{0}/MightyTerrainMesh/Resources/{1}.bytes", Application.dataPath, dataName);
            if (File.Exists(path))
                File.Delete(path);
        ...
        public static MTQuadTreeHeader LoadQuadTreeHeader(string dataName)
        {
            ...
            string path = string.Format("{0}/{1}.bytes", Application.streamingAssetsPath, dataName);
        ...
        {
            string path = string.Format("{0}/MightyTerrainMesh/Resources/{1}_{2}.bytes", Application.dataPath, dataName, data.meshId);
            if (File.Exists(path))
                File.Delete(path);
            FileStream stream = File.Open(path, FileMode.Create);
            //lods
        ...
        {
            string path = string.Format("{0}/{1}_{2}.bytes", Application.streamingAssetsPath, dataName, meshId);
            FileStream stream = File.Open(path, FileMode.Open);
            _vec2Cache.Clear();
            _intCache.Clear();
            //lods
